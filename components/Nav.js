...
export const navData = [
  { name: 'home', path: '/', icon: <HiHome key="home-icon" /> },
  { name: 'about', path: '/about', icon: <HiUser key="about-icon" /> },
  { name: 'services', path: '/services', icon: <HiRectangleGroup key="services-icon" /> },
  { name: 'contact', path: '/contact', icon: <HiEnvelope key="contact-icon" /> },
];
          ...
          return (
            <Link
              className={classNames({
                ...
                'relative flex items-center group hover:text-accent transition-all duration=300': true,
              })}
              href={link.path}
              key={index}
            >
